...
from prc_PLC_read import start_process as start_PLC_read
from prc_PLC_write import start_process as start_PLC_write
...
class main:
    def __init__(self):
        ...
        self.lst_thread_name = ["PLC_write"]
    ...
    def run(self):
        ...
        try:
            ...
            for i, str_prc_name in enumerate(self.lst_thread_name):
                ...
                self.inst_logger.info(
                    "主程序尝试启动线程: %s" % (str_thread_name,))  # 通过globlas().get 取得指定名称的入口句柄 返回给Thread作为线程启动入口
        ...
        # 启动监控线程
        self.prc_mon_thread = threading.Thread(target=globals().get("start_monitor"), args=(ini_config, lst_thread),
                                               name="start_monitor")
