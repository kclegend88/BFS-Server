...
def start_process(config_file):
    ...
    time.sleep(5)
    start_time = int(time.time())  # 使用时间戳记录启动时间
    prc_id = start_time  # 用线程启动时的时间戳作为线程标识记录在键pro_mon:<{name}>:run_lock中，每次循环判断这个值是否发生变化，如果发生变化，线程终止
    __inst_redis__.setkey(f"pro_mon:<{__prc_name__}>:run_lock", prc_id)
    __inst_redis__.setkey(f"pro_mon:<{__prc_name__}>:start_ts", start_time)
    #__inst_redis__.sadd("set_process", __prc_name__)  # 启动成功
    print("PLC线程状态修改成功")
    '''
