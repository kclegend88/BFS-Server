...
def start_process(config_file):
    ...
    __prc_cycletime = __ini_prc_config__.CycleTime.prc_monitor_cycletime
    ...
    while b_thread_running:
        ...
        # --------------------
        # 主线程操作区
        # 遍历Redis内set_process集合的所有prc_name
        current_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        lst_prc_name = inst_redis.getset("set_process")
        # 直接遍历集合
        for member in lst_prc_name:
            name = member.split('=')[1].split('/')[0]
            # 判断prc_mon:<prc_name>:run_lock是否存在，如不存在则向stm_sys-log中插入：[ts]-prcMon:<pro_name> stopped
            prc_run_lock = inst_redis.getkey(f"pro_mon:{name}:run_lock")
            if prc_run_lock is not None:
                inst_redis.lpush("stm_sys_log", f"[{current_time}]-prcMon:<{name}> stopped")
            # 判断prc_mon:<prc_name>:healthy是否为yes，如不则向stm_sys-log中插入：[ts]-prcMon:<pro_name> not healthy
            prc_healthy = inst_redis.getkey(f"pro_mon:{name}:healthy")
            if prc_healthy is not 'yes':
                inst_redis.lpush("stm_sys_log", f"[{current_time}]-prcMon:<{name}> not healthy")

            print(member)  # 使用 decode() 转换字节为字符串


        # set prc_mon:<prc_name>:healthy= yes
        #
        # 如果没有收到system stop的命令，则重新启动所有stopped的线程
        # 如果收到system stop的命令，则向stm_sys_log之外，全部线程发送exit command
        # 堵塞至stm_sys_log之外，全部线程的run_lock均消失后
        # 向stm_sys-log中插入：[ts]-prcMon:ALL process stopped, system exit at 0;
        # 向stm _sys_log现场发送exit command
        # 堵塞至stm_sys_log的run_lock消失后,退出；
