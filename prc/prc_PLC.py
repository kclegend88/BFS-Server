...
def start_process(config_file):
    ...
    def PLC_light(status):
        nonlocal __inst_plc__, inst_redis
        if status == 'on':
            inst_redis.setkey("plc_conv:light:status", "on")
            # status = int(status)
            __inst_plc__.db_write(12, 23, bytearray([1]))
        elif status == 'off':
    ...
    def prc_PLC_parcelposcalc():
        ...
        for i,key in enumerate(lst_parcel_keys):                # 遍历包裹列表
            ...
            # 在包裹流出时改变空闲停止时间
            inst_redis.setkey("plc_conv:last_activity_time", datetime.datetime.now().isoformat())
            inst_logger.debug(f"包裹已经离开CV03,uid={str_uid},barcode={str_barcode}")
            # inst_redis
            
    # 超过 ini_idle_timeout 指定时间后，无包裹通过，停止皮带机
    def prc_PLC_autosleep():
        nonlocal inst_redis, __ini_idle_timeout
        last_activity = inst_redis.getkey("plc_conv:last_activity_time")
        if not last_activity:
            return  # 从未有过活动

        last_ts = datetime.datetime.fromisoformat(last_activity)
        idle_duration = (datetime.datetime.now() - last_ts).total_seconds()
        ...
        if idle_duration > __ini_idle_timeout:
            if plc_conv_fullspeed == 'yes':
                inst_logger.info("检测到空闲超时，开始减速")
                inst_redis.setkey("plc_conv:command", "autoslowdown")
            elif plc_conv_fullspeed == 'countdown':
                inst_logger.info("低速空闲超时，完全停止")
                PLC_light("off")
                inst_redis.setkey("plc_conv:command", "stop")
                inst_redis.clearkey("plc_conv:last_activity_time", False)
                
    # 人工操作停止皮带线
    def prc_PLC_handle_operate():
        nonlocal inst_redis
        operate_cmd = inst_redis.getkey("plc_conv:operate")
        
        if not operate_cmd:
            return
        
        # 处理操作指令后立即清除
        key = inst_redis.clearkey("plc_conv:operate", False)
        
        if operate_cmd == "move":
            inst_logger.info("收到操作指令：启动输送机")
            PLC_light("on")
            inst_redis.setkey("plc_conv:command", "start")
            inst_redis.setkey("plc_conv:last_activity_time", datetime.datetime.now().isoformat())
        elif operate_cmd == "stop":
            inst_logger.info("收到操作指令：停止输送机") 
            PLC_light("off")
            inst_redis.setkey("plc_conv:command", "stop")
            inst_redis.clearkey("plc_conv:last_activity_time", False)
        else:
            inst_logger.error(f"无效操作指令：{operate_cmd}")
    ...
    __prc_cycletime=ini_config.CycleTime.prc_PLC_cycletime
    __prc_expiretime=ini_config.CycleTime.prc_PLC_expiretime
    __prc_healthytime=ini_config.CycleTime.prc_PLC_healthytime

    # --------------------    
    # 定制化配置参数读取区

    # 获取PLCIP地址，DB大小
    __plc_ip__ = ini_config.Network.PLC_server_ip
    __plc_db_size = ini_config.PLC_Config.DB3_size

    # 读取ini中 ini_speed_H， ini_speed_L， ini_conv_length
    # 读取ini中 ini_speed_H， ini_speed_L， ini_conv_length
    __ini_speed_H = ini_config.PLC_Config.ini_speed_H
    __ini_speed_L = ini_config.PLC_Config.ini_speed_L
    __ini_conv_length = ini_config.PLC_Config.ini_conv_length
    __ini_HIKOut_position = ini_config.PLC_Config.ini_HIKOut_position
    __ini_stop_position_offset = ini_config.PLC_Config.ini_stop_position_offset

    __ini_start_conv = ini_config.PLC_Config.StartConv
    # 读取ini中 ini_idle_timeout ，皮带线超时停止时间
    __ini_idle_timeout = ini_config.PLC_Config.ini_idle_timeout
    ...
    while b_thread_running:
        try:
            ...
            # 新增空闲检测逻辑（超时自动停止）
            if inst_redis.getkey("plc_conv:status") == 'run':
                prc_PLC_autosleep()
