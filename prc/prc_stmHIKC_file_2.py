...
def start_process(config_file):
    def stmHIKC_file_download(stream_test):
        for i, dictdata in stream_test:  # 遍历收到的所有消息
            ...
            if os.path.exists(path):
                ...
                for filename, _ in files_sorted:
                    ...
                    if uid in filename:  # 如果文件名中包含uid
                        ...
                        return True  # 如果只需要找到一个图片就可以退出循环
                    else:
                        if x == max_number:
                            inst_logger.info(f"图片遍历前30个，uid:{uid}未找到，该uid的result:{result}")
                            if result != 'GR':
                                return False
                        x = x + 1
    ...
    def stmHIKC_file_delete(stream_delete):
        for i, dictdata in stream_delete:
            uid = dictdata['uid']
                ...
                for filename in os.listdir(order_path):
                    if uid in filename:  # 如果文件名中包含uid
                        ...
                        try:
                            # 删除此文件
                            os.remove(full_path)
                            inst_logger.info(f"图片 {filename} 已删除")
                            return True
                        except Exception as e:
                            inst_logger.error(f"图片 {filename} 删除失败,发生错误{traceback.format_exc()}")
                            return False
                inst_logger.info(f"图片{uid}未找到 删除失败！")

    __prc_name__ = "stmHIKC_file_2"  ### 需手动配置成线程名称
    ...
    __prc_cycletime = ini_config.CycleTime.prc_stimHIKCfile_cycletime
    __prc_expiretime = ini_config.CycleTime.prc_stimHIKCfile_expiretime
    __prc_healthytime = ini_config.CycleTime.prc_stimHIKCfile_healthytime
    max_number = ini_config.find_maxnumber.max_number

    # --------------------    
    # 定制化配置参数读取区
    # HIK图片位置
    AIRead_path = ini_config.Image_Source_Path.AIRead_path
    ErrRead_path = ini_config.Image_Source_Path.ErrRead_path
    NoRead_path = ini_config.Image_Source_Path.NoRead_path
    # 存储图片位置
    AITarget_path = ini_config.Image_Target_Path.AITarget_path
    ErrTarget_path = ini_config.Image_Target_Path.ErrTarget_path
    NoTarget_path = ini_config.Image_Target_Path.NoTarget_path
    # 定制化配置参数读取区
    # --------------------
    ...
    __ftp_consumer_group__ = "severFtp_consumerGroup"
    re_list = []  # 记录未找到图片的uid
    # 检查并创建消费组
    group_name = "stmHIKC_file"
    try:
        inst_redis.xcreategroup(stream_name_create, group_name)
        inst_logger.info("线程 %s 注册stream组成功" % (__prc_name__,))
    except Exception as e:
        inst_logger.info("线程 %s 注册stream组失败，该组已存在" % (__prc_name__,))
    try:
        inst_redis.xcreategroup(stream_name_delete, group_name)
        inst_logger.info("线程 %s 注册stream组成功" % (__prc_name__,))
    except Exception as e:
        inst_logger.info("线程 %s 注册stream组失败，该组已存在" % (__prc_name__,))
    # 以上为定制初始化区域
    ...
    while b_thread_running:
        ...
        if re_list:
            for i in re_list:
                # 找图片  进行转移，如果找不到，则记录uid，删除uid
                result = stmHIKC_file_download(i)
                if result:
                    inst_logger.info(f"re_list中图片已找到")
                else:
                    inst_logger.info(f"处于re_list中图片未找到，从list移除该信息{i}")
                re_list.remove(i)
        # --------------------
        # 以下为主线程操作区
        # 获取stream_test，stream_reading_confirm
        stream_test = inst_redis.xreadgroup(stream_name_create, group_name, __prc_name__)
        if stream_test:
            inst_logger.info("收到创建序列 %s 中的消息累计 %d 行" % (stream_test[0][0], len(stream_test[0][1])))
            inst_logger.info(f"2号ftp进行下载")
            result = stmHIKC_file_download(stream_test[0][1])
            if result:
                inst_logger.info(f"图片已找到")
            else:
                inst_logger.info(f"图片未找到，添加至list{stream_test[0][1]}")
                re_list.append(stream_test[0][1])
        # 删除
        stream_delete = inst_redis.xreadgroup(stream_name_delete, group_name, __prc_name__)
        if stream_delete:
            inst_logger.info("收到删除序列 %s 中的消息累计 %d 行" % (stream_delete[0][0], len(stream_delete[0][1])))
            stmHIKC_file_delete(stream_delete[0][1])
        # 以上为主线程操作区       
        # --------------------
