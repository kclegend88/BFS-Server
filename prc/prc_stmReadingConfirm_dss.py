...
def start_process(config_file):
    ...
    __prc_cycletime=ini_config.CycleTime.prc_stmReadingConfirmdss_cycletime
    __prc_expiretime=ini_config.CycleTime.prc_stmReadingConfirmdss_expiretime
    __prc_healthytime=ini_config.CycleTime.prc_stmReadingConfirmdss_healthytime

    # --------------------    
    # 定制化配置参数读取区
    # ccr = __ini_prc_config__.Dss.ccr
    dss_ip = ini_config.Network.DSS_IP
    dss_port = ini_config.Network.DSS_Port
    dss_push = ini_config.DSS_Push.MODE
        ...
        if stream_readingConfirm_dss:
            ...
            for i, dictdata in stream_readingConfirm_dss[0][1]:
                ...
                if check_result != sys_opmode :
                    inst_logger.error(
                        f"barcode:{barcode}包裹与当前操作模式 {sys_opmode} 不符,check_result:{check_result}")
                    continue
                result_menu = [{"hawb": barcode, "ccr": sys_opmode}]
